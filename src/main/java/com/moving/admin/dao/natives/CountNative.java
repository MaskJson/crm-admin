...
public class CountNative extends AbstractNative {
    ...
    public List<Map<String, Object>> getProjectStatusTalents(Long userId) {
        ...
        String where = " where (pr.create_user_id=" + userId + " or pt.talent_id in(select distinct tr.talent_id from talent_remind tr where tr.create_user_id=" + userId + ")) and pr.status in (1,3,5,7) ";
    ...
    public Map<String, Object> getReportPending(Long userId, Pageable pageable) {
        ...
        String where = " where r.status=1 and r.type=1 and p.advise_id="+userId+" and p.team_id is not null and p.team_id in " + inStr;
