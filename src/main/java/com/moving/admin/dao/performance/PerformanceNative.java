...
public class PerformanceNative extends AbstractNative {
    ...
    private String select = "select pr.id as remindId, pr.status as remindStatus, pt.id, pt.project_id as projectId, pt.talent_id as talentId, pt.status, pt.type, pt.create_time as createTime, pt.update_time as updateTime," +
    ...
    public List<Map<String, Object>> getPerformance(Long userId, Integer flag, String time) {
        ...
        return getProjectStatusTalents(where + userId + whereStr, userId);
    }
    // 周、月报报表
    public List<Map<String, Object>> getPerformanceReport(Long userId, Long roleId, Integer flag, String time) {
        ...
        switch (flag) {
            case 1:where = where + " and YEARWEEK(date_format(pr.create_time, '%Y-%m-%d')) = YEARWEEK("+time+", '%Y-%m-%d')";break;
            case 2:where = where + " and DATE_FORMAT(pr.create_time, '%Y%m') = DATE_FORMAT("+time+", '%Y%m')";break;
        }
        return getProjectStatusTalents(where, userId);
    }

    public List<Map<String, Object>> getProjectStatusTalents(String where, Long userId) {
...
//            map.put("info", countNative.getWorkInfo(id));
...
//            map.put("remind", countNative.getRemindInfo(id));
